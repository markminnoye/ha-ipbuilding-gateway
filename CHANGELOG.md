...
- **Versie-header onderaan** — vanaf 1.8.0 staat de `# ipbuilding_blueprint_version: N`-regel onderaan de blueprint-file in kleine letters, niet meer bovenaan. De sync in `blueprints.py` scant nu de hele file. De `**Blueprint-versie: N.**`-marker in de description (operator-zichtbaar in HA) blijft ongewijzigd.
